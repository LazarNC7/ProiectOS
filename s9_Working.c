...
void countCorrectSentences(const char *filename, char character, int pipe_fd) {
    FILE *fp;
    char command[100];
    int count;

    // Construct the command to send the file content and character to the Bash script
    snprintf(command, sizeof(command), "bash scriptS9.sh %c<%s", character, filename);
    ...
    // Create a buffer to store the output of the script
  ...
  char file_path[PATH_MAX];
	...
	if (isBMPFilePath(file_path)) {
	  ...
	  if (count_pid == 0) {
	    ...
	    countCorrectSentences(file_path, character, pipe_fd[1]);
